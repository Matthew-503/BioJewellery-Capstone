...
//Get user account
export const getAccount = createAsyncThunk('auth/get', async (_, thunkAPI) => {
  try {
      const token = thunkAPI.getState().auth.user.token
      return await authService.getAccount(token)
  } catch (error) {
...
export const updateAccount = createAsyncThunk('auth/update', async (userData, thunkAPI) => {
  try {
    return await authService.updateAccount(userData)
  } catch (error) {
      const message = (
          error.response &&
          error.response.data &&
